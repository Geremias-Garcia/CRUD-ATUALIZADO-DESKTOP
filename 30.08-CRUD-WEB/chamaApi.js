...
document.addEventListener("click", function (event) {
    if (event.target && event.target.classList.contains("btn-edit")) {
      const idUsuario = event.target.dataset.id; // Obtém o ID do usuário selecionado
      document.documentElement.dataset.idUsuario = idUsuario;
      recuperarUsuarioPorId(idUsuario);
    }
});

function recuperarUsuarioPorId(idUsuario) {
    axios
      .get(`http://localhost:3000/pegarUsuarioPeloId/${idUsuario}`)
      .then((response) => {
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    const idUsuario = document.documentElement.dataset.idUsuario;
    ...
    const novaDisciplina = document.querySelector(
      "#editarUsuario textarea"
    ).value;

    if (novoNome && novoEmail && novaDisciplina) {
      // Chame a função editarUsuario com os novos dados
      editarUsuario(idUsuario, novoNome, novoEmail, novaDisciplina);
...
function editarUsuario(idUsuario, novoNome, novoEmail, novaDisciplina) {
  axios
    .put(`http://localhost:3000/atualizarUsuario/${idUsuario}`, {
      nome: novoNome,
      email: novoEmail,
